...
try {
    ...
    console.log("🌍 Connected to the Ultrons DB 🍃")
} catch {
...
let cc = Config.sessionName.replace(/Ultron;;;/g, "");
async function MakeSession(){
if (!fs.existsSync(__dirname + '/ultroid/creds.json')) {
    if(cc.length<30){
    const axios = require('axios');
    let { data } = await axios.get('https://paste.c-net.org/'+cc)
    await fs.writeFileSync(__dirname + '/ultroid/creds.json', atob(data), "utf8")    
    } else {
	 var c = atob(cc)
         await fs.writeFileSync(__dirname + '/ultroid/creds.json', c, "utf8")    
    }
}
}
MakeSession()
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        const ChangePic = __dirname + "/assets/Multidevice.png"
        await writeFile(ChangePic, thumbbuffer);
        global.log0 = fs.readFileSync(__dirname + "/assets/Multidevice.png"); //ur logo pic
        const { state, saveCreds } = await useMultiFileAuthState(__dirname + '/ultroid/')
        const Void = VoidConnect({
            logger: pino({ level: 'fatal' }),
            printQRInTerminal: true,
            browser: ['Ultron', 'safari', '1.0.0'],
    ...
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
		 ...
		 if (Config.readmessage==="true" && icmd) {
                ...
                if (citel.chat === "120363039839467167@g.us") return
...
<html>
  ...
  <body>
    <section>
      Hello from United Data Group!<br>
      Ultron Multidevice Whatsapp Bot<br>
      (c) 2024 [ Mugwe Jnr ]
    </section>
  </body>
</html>
